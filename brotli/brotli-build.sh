#!/bin/bash
# This script downlaods and builds the Mac, iOS and tvOS brotli libraries 
#
# Credits:
# Jason Cox, @jasonacox
...
trap 'echo -e "${alert}** ERROR with Build - Check /tmp/brotli*.log${alertdim}"; tail -5 /tmp/brotli*.log' INT TERM EXIT
...
usage ()
{
	...
	echo
    echo -e "  ${subbold}$0${normal} [-v ${dim}<brotli version>${normal}] [-s ${dim}<iOS SDK version>${normal}] [-t ${dim}<tvOS SDK version>${normal}] [-m] [-x] [-h]"
    echo
	echo "         -v   version of brotli (default $BROTLI_VERNUM)"
...
BROTLI_VERSION="brotli-${BROTLI_VERNUM}"
...
if (type "pkg-config" > /dev/null 2>&1 ) ; then
	...
	if (type "brew" > /dev/null 2>&1 ) ; then
		...
		./configure-cmake --prefix=/tmp/pkg_config --with-internal-glib >> "/tmp/${BROTLI_VERSION}.log" 2>&1
		make -j${CORES} >> "/tmp/${BROTLI_VERSION}.log" 2>&1
		make install >> "/tmp/${BROTLI_VERSION}.log" 2>&1
...
buildMac()
{
	...
	export PASSTHRU="--pass-thru -G Xcode -DCMAKE_TOOLCHAIN_FILE=../ios.toolchain.cmake -DARCHS=$ARCH"
	...
	echo -e "${subbold}Building ${BROTLI_VERSION} for ${archbold}${ARCH}${dim} (MacOS ${MACOS_VER})"

	pushd "${BROTLI_VERSION}" > /dev/null

	if [[ $ARCH != ${BUILD_MACHINE} ]]; then
		# cross compile required
		if [[ "${ARCH}" == "arm64" || "${ARCH}" == "arm64e"  ]]; then
			./configure-cmake --disable-debug --prefix="${BROTLI}/Mac/${ARCH}" $PASSTHRU &> "/tmp/${BROTLI_VERSION}-${ARCH}.log"
		else
			./configure-cmake --disable-debug --prefix="${BROTLI}/Mac/${ARCH}" $PASSTHRU &> "/tmp/${BROTLI_VERSION}-${ARCH}.log"
		fi
	else
	...
	make -j${CORES} lib >> "/tmp/${BROTLI_VERSION}-${ARCH}.log" 2>&1
	mkdir -p ${BROTLI}/Mac/${ARCH}/lib
	cp libbrotli.a ${BROTLI}/Mac/${ARCH}/lib && rm libbrotli.a
	cp -R c/include ${BROTLI}/Mac/${ARCH}/
	make clean >> "/tmp/${BROTLI_VERSION}-${ARCH}.log" 2>&1
	popd > /dev/null
...
buildCatalyst()
{
    ...
    export CFLAGS="-arch ${ARCH} -pipe -Os -gdwarf-2 -fembed-bitcode"
    export LDFLAGS="-arch ${ARCH}"
	export PASSTHRU="--pass-thru -G Xcode -DCMAKE_TOOLCHAIN_FILE=../ios.toolchain.cmake -DARCHS=$ARCH -DDEPLOYMENT_TARGET=${CATALYST_IOS}"
	...
	echo -e "${subbold}Building ${BROTLI_VERSION} for ${archbold}${ARCH}${dim} (MacOS ${MACOS_VER} Catalyst iOS ${CATALYST_IOS})"

	pushd "${BROTLI_VERSION}" > /dev/null

	# Cross compile required for Catalyst
	if [[ "${ARCH}" == "arm64" ]]; then
		./configure-cmake --prefix="${BROTLI}/Catalyst/${ARCH}" $PASSTHRU &> "/tmp/${BROTLI_VERSION}-catalyst-${ARCH}.log"
	else
		./configure-cmake --prefix="${BROTLI}/Catalyst/${ARCH}" $PASSTHRU &> "/tmp/${BROTLI_VERSION}-catalyst-${ARCH}.log"
	fi
	
	make -j${CORES} lib >> "/tmp/${BROTLI_VERSION}-catalyst-${ARCH}.log" 2>&1
	mkdir -p ${BROTLI}/Catalyst/${ARCH}/lib
	cp libbrotli.a ${BROTLI}/Catalyst/${ARCH}/lib && rm libbrotli.a
	cp -R c/include ${BROTLI}/Catalyst/${ARCH}/
	make clean >> "/tmp/${BROTLI_VERSION}-catalyst-${ARCH}.log" 2>&1
	popd > /dev/null
...
buildIOS()
{
	...
	export PASSTHRU="--pass-thru -G Xcode -DCMAKE_TOOLCHAIN_FILE=../ios.toolchain.cmake"
   	export PASSTHRU="$PASSTHRU -DARCHS=$ARCH -DPLATFORM=OS -DDEPLOYMENT_TARGET=${IOS_MIN_SDK_VERSION}"
	echo -e "${subbold}Building ${BROTLI_VERSION} for ${PLATFORM} ${IOS_SDK_VERSION} ${archbold}${ARCH}${dim} (iOS ${IOS_MIN_SDK_VERSION})"
	
	if [[ "${ARCH}" == "arm64" || "${ARCH}" == "arm64e"  ]]; then
		./configure-cmake --prefix="${BROTLI}/iOS/${ARCH}" $PASSTHRU &> "/tmp/${BROTLI_VERSION}-iOS-${ARCH}-${BITCODE}.log"
	else
		./configure-cmake --prefix="${BROTLI}/iOS/${ARCH}" $PASSTHRU &> "/tmp/${BROTLI_VERSION}-iOS-${ARCH}-${BITCODE}.log"
	fi

	make -j4 lib >> "/tmp/${BROTLI_VERSION}-iOS-${ARCH}-${BITCODE}.log" 2>&1
	mkdir -p ${BROTLI}/iOS/$ARCH/lib
	cp libbrotli.a ${BROTLI}/iOS/$ARCH/lib && rm libbrotli.a
	cp -R c/include ${BROTLI}/iOS/$ARCH/
	make clean >> "/tmp/${BROTLI_VERSION}-iOS-${ARCH}-${BITCODE}.log" 2>&1
	popd > /dev/null
...
buildIOSsim()
{
	...
	echo -e "${subbold}Building ${BROTLI_VERSION} for ${PLATFORM} ${IOS_SDK_VERSION} ${archbold}${ARCH}${dim} (iOS ${IOS_MIN_SDK_VERSION})"
	if [[ "${ARCH}" == "arm64" || "${ARCH}" == "arm64e"  ]]; then
		./configure-cmake --prefix="${BROTLI}/iOS-simulator/${ARCH}" $PASSTHRU &> "/tmp/${BROTLI_VERSION}-iOS-${ARCH}-${BITCODE}.log"
	else
		./configure-cmake --prefix="${BROTLI}/iOS-simulator/${ARCH}" $PASSTHRU &> "/tmp/${BROTLI_VERSION}-iOS-${ARCH}-${BITCODE}.log"
	fi

	make -j8 lib>> "/tmp/${BROTLI_VERSION}-iOS-${ARCH}-${BITCODE}.log" 2>&1
	mkdir -p ${BROTLI}/iOS-simulator/$ARCH/lib
	cp libbrotli.a ${BROTLI}/iOS-simulator/$ARCH/lib && rm libbrotli.a
	cp -R c/include ${BROTLI}/iOS-simulator/$ARCH/
	make clean >> "/tmp/${BROTLI_VERSION}-iOS-${ARCH}-${BITCODE}.log" 2>&1
	popd > /dev/null
...
buildTVOS()
{
	...
	export LDFLAGS="-arch ${ARCH} -isysroot ${CROSS_TOP}/SDKs/${CROSS_SDK} ${BROTLILIB}"
	export LC_CTYPE=C

	export PASSTHRU="--pass-thru -G Xcode -DCMAKE_TOOLCHAIN_FILE=../ios.toolchain.cmake"
	export PASSTHRU="$PASSTHRU -DDEPLOYMENT_TARGET=${TVOS_MIN_SDK_VERSION} -DARCHS=$ARCH -DPLATFORM=TVOS"
  
	echo -e "${subbold}Building ${BROTLI_VERSION} for ${PLATFORM} ${TVOS_SDK_VERSION} ${archbold}${ARCH}${dim} (tvOS ${TVOS_MIN_SDK_VERSION})"
	...
	./configure-cmake --prefix="${BROTLI}/tvOS/${ARCH}" $PASSTHRU &> "/tmp/${BROTLI_VERSION}-tvOS-${ARCH}.log"
	...
	make -j8 lib>> "/tmp/${BROTLI_VERSION}-tvOS-${ARCH}-${BITCODE}.log" 2>&1
	mkdir -p ${BROTLI}/tvOS/$ARCH/lib
	cp libbrotli.a ${BROTLI}/tvOS/$ARCH/lib && rm libbrotli.a
	cp -R c/include ${BROTLI}/tvOS/$ARCH/
	make clean >> "/tmp/${BROTLI_VERSION}-tvOS-${ARCH}-${BITCODE}.log" 2>&1
...
buildTVOSsim()
{
	...
	export LDFLAGS="-arch ${ARCH} -isysroot ${SYSROOT} ${BROTLILIB}"
	export LC_CTYPE=C

	export PASSTHRU="--pass-thru -G Xcode -DCMAKE_TOOLCHAIN_FILE=../ios.toolchain.cmake"
	export PASSTHRU="$PASSTHRU -DDEPLOYMENT_TARGET=${TVOS_MIN_SDK_VERSION} -DARCHS=$ARCH -DPLATFORM=SIMULATOR_TVOS"

	echo -e "${subbold}Building ${BROTLI_VERSION} for ${PLATFORM} ${TVOS_SDK_VERSION} ${archbold}${ARCH}${dim} (tvOS Simulator ${TVOS_MIN_SDK_VERSION})"
	...
	if [[ "${ARCH}" == "arm64" ]]; then
		./configure-cmake --prefix="${BROTLI}/tvOS-simulator/${ARCH}" $PASSTHRU &> "/tmp/${BROTLI_VERSION}-tvOS-simulator${ARCH}.log"
	else
		./configure-cmake --prefix="${BROTLI}/tvOS-simulator/${ARCH}" $PASSTHRU &> "/tmp/${BROTLI_VERSION}-tvOS-simulator${ARCH}.log"
	fi
	...
	make -j8 lib>> "/tmp/${BROTLI_VERSION}-tvOS-${ARCH}-${BITCODE}.log" 2>&1
	mkdir -p ${BROTLI}/tvOS-simulator/$ARCH/lib
	cp libbrotli.a ${BROTLI}/tvOS-simulator/$ARCH/lib && rm libbrotli.a
	cp -R c/include ${BROTLI}/tvOS-simulator/$ARCH/
	make clean >> "/tmp/${BROTLI_VERSION}-tvOS-${ARCH}-${BITCODE}.log" 2>&1
...
rm -rf "/tmp/${BROTLI_VERSION}-*"
rm -rf "/tmp/${BROTLI_VERSION}-*.log"

rm -rf "${BROTLI_VERSION}"

if [ ! -e ${BROTLI_VERSION}.tar.gz ]; then
	echo "Downloading ${BROTLI_VERSION}.tar.gz"
	curl -Ls https://github.com/google/brotli/archive/refs/tags/v${BROTLI_VERNUM}.tar.gz -o "${BROTLI_VERSION}.tar.gz"
else
	echo "Using ${BROTLI_VERSION}.tar.gz"
fi

echo "Unpacking brotli"
tar xfz "${BROTLI_VERSION}.tar.gz"
...
lipo \
        "${BROTLI}/Mac/x86_64/lib/libbrotli.a" \
		"${BROTLI}/Mac/arm64/lib/libbrotli.a" \
        -create -output "${BROTLI}/lib/libbrotli_Mac.a"
...
lipo \
        "${BROTLI}/Catalyst/x86_64/lib/libbrotli.a" \
		"${BROTLI}/Catalyst/arm64/lib/libbrotli.a" \
        -create -output "${BROTLI}/lib/libbrotli_Catalyst.a"
fi
...
lipo \
	"${BROTLI}/iOS/armv7/lib/libbrotli.a" \
	"${BROTLI}/iOS/armv7s/lib/libbrotli.a" \
	"${BROTLI}/iOS-simulator/i386/lib/libbrotli.a" \
	"${BROTLI}/iOS/arm64/lib/libbrotli.a" \
	"${BROTLI}/iOS/arm64e/lib/libbrotli.a" \
	"${BROTLI}/iOS-simulator/x86_64/lib/libbrotli.a" \
	-create -output "${BROTLI}/lib/libbrotli_iOS-fat.a"
...
lipo \
	...
	"${BROTLI}/iOS/arm64/lib/libbrotli.a" \
	"${BROTLI}/iOS/arm64e/lib/libbrotli.a" \
	-create -output "${BROTLI}/lib/libbrotli_iOS.a"

lipo \
	"${BROTLI}/iOS-simulator/i386/lib/libbrotli.a" \
	"${BROTLI}/iOS-simulator/x86_64/lib/libbrotli.a" \
	"${BROTLI}/iOS-simulator/arm64/lib/libbrotli.a" \
	-create -output "${BROTLI}/lib/libbrotli_iOS-simulator.a"
...
lipo \
        "${BROTLI}/tvOS/arm64/lib/libbrotli.a" \
        -create -output "${BROTLI}/lib/libbrotli_tvOS.a"
...
lipo \
        "${BROTLI}/tvOS/arm64/lib/libbrotli.a" \
        "${BROTLI}/tvOS-simulator/x86_64/lib/libbrotli.a" \
        -create -output "${BROTLI}/lib/libbrotli_tvOS-fat.a"

lipo \
	"${BROTLI}/tvOS-simulator/x86_64/lib/libbrotli.a" \
	"${BROTLI}/tvOS-simulator/arm64/lib/libbrotli.a" \
	-create -output "${BROTLI}/lib/libbrotli_tvOS-simulator.a"
